...
<body>
    ...
    <div class="mb-4">
        <h2 class="d-flex align-items-center" id="author_name">
            <i class="bi bi-person-circle me-2"></i> Author
        </h2>
    </div>

    <nav class="flex-grow-1">
        <a href="{{ url('/author/dashboard') }}" 
           class="d-flex align-items-center mb-2 px-2 py-1 text-white {{ request()->is('author/dashboard') ? 'active' : '' }}">
            <i class="bi bi-speedometer2 me-2"></i> Dashboard
        </a>
        <a href="{{ url('/author/posts') }}" 
           class="d-flex align-items-center mb-2 px-2 py-1 text-white {{ request()->is('author/posts*') ? 'active' : '' }}">
    ...
    <script>
        ...
        async function fetchUserInfo() {
            ...
            document.getElementById('author_name').textContent = `${user.data?.name ?? 'Author'}`;
            } catch (err) {
